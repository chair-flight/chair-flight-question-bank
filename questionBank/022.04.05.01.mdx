---
SyllabusReference: 022.04.05.01
Title:
  Operating principles, components, comparison with a direct-reading magnetic
  compass
---

# Operating principles, components, comparison with a direct-reading magnetic compass

## Learning Objectives

1. **022.04.05.01.01** -- Describe the principles of operation of a remote-reading
   compass system.
2. **022.04.05.01.02** -- Using a block diagram, list and explain the function of
   the following components of a remote-reading compass system: flux detection
   unit; gyro unit; transducers, precession amplifiers, annunciator; display
   unit (compass card, synchronising and set-heading knob, DG/compass/slave/free
   switch).
3. **022.04.05.01.03** -- State the advantages and disadvantages of a
   remote-reading compass system compared to a direct-reading magnetic compass
   with regard to: design (power source, weight and volume); deviation due to
   aircraft magnetism; turning and acceleration errors; attitude errors;
   accuracy and stability of the information displayed; availability of the
   information for several systems (compass card, RMI, automatic flight control
   system (AFCS)).

|                 | ATPL(A) | CPL(A) | ATPL(H)/IR | ATPL(H)/VFR | CPL(H) | IR  | CBIR(A) |
| --------------- | ------- | ------ | ---------- | ----------- | ------ | --- | ------- |
| 022.04.05.01.01 | X       | X      | X          | X           | X      | X   | X       |
| 022.04.05.01.02 | X       | X      | X          | X           | X      | X   | X       |
| 022.04.05.01.03 | X       | X      | X          | X           | X      | X   |         |

## Summary

## Questions

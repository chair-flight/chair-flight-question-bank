---
SyllabusReference: 031.04.01.07
Title: Details of fuel system relevant to mass-and-balance considerations
---

# Details of fuel system relevant to mass-and-balance considerations

## Learning Objectives

1. **031.04.01.07.01** -- Extract the appropriate data (e.g. fuel-tank capacities
   and fuel-tank positions) from given sample documents.
2. **031.04.01.07.02** -- Explain aircraft CG movement as flight progresses given
   location of fuel tank (inner wing, outer wing, central, additional aft
   central, horizontal stabiliser) and mass of fuel consumed from that tank and
   aeroplane"s previous CG.
3. **031.04.01.07.03** -- Explain advantages and risks associated with fuel tanks
   in the aeroplane"s fin or horizontal stabiliser.

|                 | ATPL(A) | CPL(A) | ATPL(H)/IR | ATPL(H)/VFR | CPL(H) | IR  | CBIR(A) |
| --------------- | ------- | ------ | ---------- | ----------- | ------ | --- | ------- |
| 031.04.01.07.01 | X       | X      | X          | X           | X      |     |         |
| 031.04.01.07.02 | X       |        |            |             |        |     |         |
| 031.04.01.07.03 | X       |        |            |             |        |     |         |

## Summary

## Questions
